...
画面はすべて `/mayuchan` 以下にあり、日本語表示です。お客様向けの公開画面(`/`, `/search`, `/restaurants/...`)は多言語対応です。
...
1. 各管理画面の上部にある「運用者トークン」欄にトークンを入力し、右の**「認証」ボタン**を押します。
   - サーバでトークンの有効性を確認し(`GET /api/mayuchan/auth`)、有効な場合のみページの内容が表示されます。
   - 入力値は**この端末のセッション(sessionStorage)**にのみ保存され、サーバには保存されません。
   - ブラウザのタブを閉じると消えます。再度入力してください。保存済みトークンがあるページ間の移動では自動で再検証されます。
2. `ADMIN_TOKEN` が**未設定の環境では、管理APIはすべて拒否**されます(開発環境も同じ)。
   - サーバログに `[admin] ADMIN_TOKEN is not set; rejecting all admin API requests.` のエラーが出ます。
3. **開発・本番を問わず `ADMIN_TOKEN` を設定**してください(→ [9. 環境変数](#9-環境変数運用設定))。ローカルは `.env.local` に記載します(雛形: `.env.example`)。
...
### 3.2 1店舗ずつ登録 `/mayuchan/restaurants/new`
...
- **写真**:
  ...
  - 「メイン」ラジオで主写真を1枚指定。
- **営業時間**: 曜日 + 開店/閉店 + 備考の行を追加。**1日に何行でも**登録できます(昼/夜の二部営業など)。同じ曜日で時間帯が重なっている場合は**警告**を表示します(修正を促すのみで、そのまま保存も可能)。閉店が開店より早い行は翌日までの深夜営業として扱います。

> 編集内容は**操作ログに差分(変更前→変更後)**として記録されます(→ [7. 操作ログ](#7-操作ログ監査)。

---

## 4. CSVインポート `/mayuchan/restaurants/import`
...
- **状態タブ**: 未対応 / 代替提案 / 確定 / 完了 / お断り / キャンセル / すべて。これらは併用できます。
- **1ページ50件**で表示します(検索・タブを変えると1ページ目に戻ります)。
- 一覧は**受付日時の新しい順**です(新しく届いたリクエストが先頭に来ます)。
...
| 代替提案(counter_offer) | 確定する / お断り / キャンセル / 未対応に戻す |
| 確定(confirmed) | 来店済み(completed)/ No-show / キャンセル / 未対応に戻す |
| お断り・キャンセル | 未対応に戻す |
| 完了・No-show | (終端: 変更不可) |

> **未対応に戻す**は誤操作の取り消し用です。この操作では**お客様への通知は送られません**。画面上部の「ステータスの説明」にも各状態の意味を記載しています。
...
## 6. 口コミ モデレーション `/mayuchan/reviews`

通報された口コミや非表示中の口コミを確認し、対応します。

- フィルタ: **通報あり** / **非表示中** / すべて。**1ページ50件**で表示します。
...
## 7. 操作ログ(監査)`/mayuchan/audit`

管理操作の履歴を確認できます。

- 記録対象: 店舗の登録・編集・状態変更、CSV取込、予約対応、口コミ対応、画像整理。
- 各エントリに **操作種別・要約・操作者(トークンのハッシュ)・IP・日時** を表示。一覧では変更された**項目名**をチップで表示します。
- アクション種別で絞り込み可能。
- **1ページ100件**で表示します。件数が多い場合は一覧の上下の「前へ/次へ」やページ番号で移動してください(絞り込みを変えると1ページ目に戻ります)。

### 7.1 操作ログ詳細 `/mayuchan/audit/<id>`

一覧の行(または「詳細 →」)をクリックすると詳細画面が開き、**どの項目がどう変更されたか**を確認できます。

- **変更内容**: 項目ごとの「変更前 → 変更後」のテーブル。値は省略せず全文表示します。
  - 店舗編集は全編集項目を比較。**写真・営業時間は内容まで比較**し、追加行(緑)・削除行(赤)を色分けします(同じ枚数での差し替えも検知)。
  - 店舗の状態変更・予約対応・口コミ対応も「変更前 → 変更後」を記録します。
  - 店舗登録は投入した初期値を「(なし)→ 値」の形で記録します。
- **補足情報**: CSV取込の件数(新規/更新/失敗)、画像整理の削除件数・解放容量、予約対応のメモなど。
- **記録の生データ(JSON)**: 調査用に detail の生 JSON も確認できます。

> 過去の記録(この機能の追加前)は旧形式のため、詳細画面では要約と生データのみ表示される場合があります。

---

## 8. メンテナンス: 孤立画像の掃除 `/mayuchan/maintenance`
...
| `ADMIN_TOKEN` | 管理API保護トークン | **未設定だと管理APIは常に拒否(開発環境でも必須)** |
...
| 12 | **死活監視 / スリープ防止** | UptimeRobot(または同種の監視サービス) | UptimeRobot のコンソール(監視URL・通知先メール。env外) | 無料(50モニタ・監視間隔5分まで) | 本番で推奨(Render 無料プランのスリープ防止と Supabase の休止対策を兼ねる) |
...
| 症状 | 確認・対処 |
|---|---|
| 管理画面で「認証が必要です」 | 上部の運用者トークンが `ADMIN_TOKEN` と一致しているか確認。サーバ側で `ADMIN_TOKEN` が未設定の場合も常に拒否されます。 |
